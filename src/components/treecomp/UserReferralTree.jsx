...
const UserReferralTree = () => {
  const [users, setUsers] = useState([]);
  const [rootId, setRootId] = useState(null); // Will set from token
  const [steps, setSteps] = useState(3);
  const [paginationIndex, setPaginationIndex] = useState(0);
  ...
  // ✅ Get logged-in user's referral code from token
  useEffect(() => {
    const token = localStorage.getItem("authToken");
    if (token) {
      const decoded = jwtDecode(token);
      const userCode = decoded.code; // must match how you encode
      setRootId(userCode);
      setNavigationPath([userCode]);
    }
  }, []);
  ...
  const childrenMap = useMemo(() => {
    ...
    users.forEach((user) => {
      ...
      if (!map.has(user.referralCode)) map.set(user.referralCode, []);
      map.get(user.referralCode).push(user);
    });
    return map;
  }, [users]);

  const handleUserClick = (id, currentDepth) => {
    const newDepth = currentDepth + 1;
    if (newDepth > MAX_DEPTH - 1) return; // ❌ Prevent going beyond 7 levels
    setRootId(id);
    setNavigationPath((prev) => [...prev, id]);
    setPaginationIndex(0);
  };
  ...
  const goToFirstChild = () => {
    const firstChild = childrenMap.get(rootId)?.[0];
    if (firstChild) handleUserClick(firstChild.code, navigationPath.length - 1);
  };

  const buildTreeAndConnections = (id, depth, connections = [], currentDepth = 0) => {
    if (depth === 0 || !childrenMap.has(id) || currentDepth >= MAX_DEPTH) return [];

    const children = childrenMap.get(id).slice(paginationIndex * 2, paginationIndex * 2 + 2);

    return children.map((child) => {
      connections.push({ from: id, to: child.code });
      return {
        id: child.code,
        children: buildTreeAndConnections(child.code, depth - 1, connections, currentDepth + 1),
  ...
  const { tree, connections } = useMemo(() => {
    ...
    const tree = [
      {
        id: rootId,
        children: buildTreeAndConnections(rootId, steps, conn, navigationPath.length - 1),
        ...
        <div className="flex justify-center space-x-6 relative">
          {nodes.map((node) => (
            <div key={node.id} className="relative flex flex-col items-center">
              <UserInTree
                id={node.id}
                onClick={() => handleUserClick(node.id, node.depth)}
                size={boxSize}
              />
              {node.children.length > 0 && <div className="w-px h-6 my-1" />}
              <div className="flex space-x-6">{renderTree(node.children)}</div>
  ...
  if (!rootId) return null; // Still loading token

  return (
    <div className="h-[calc(100vh-64px)] overflow-hidden flex flex-col bg-black text-gray-800 scroll-smooth pt-0">
      <div className="flex-1 w-full flex flex-col justify-between items-center py-6">
        <div className="flex flex-col">
          <h1 className="h-8 text-center text-3xl font-bold mb-8 text-yellow-700">My Team</h1>
        ...
        <div className="mt-12 w-full max-w-3xl mx-auto bg-[#272727] rounded-xl shadow-lg p-6 border border-gray-700 text-white flex flex-wrap justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {[3, 5, 7].map((val) => (
              <button
                ...
                onClick={() => setSteps(val)}
                className={`w-10 h-10 rounded-full text-sm font-semibold flex items-center justify-center transition 
                ${steps === val ? "bg-yellow-400 text-black" : "bg-gray-700 hover:bg-yellow-600"}`}
              >
                {val}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => setPaginationIndex((prev) => Math.min(prev + 1, totalPages - 1))}
              disabled={paginationIndex >= totalPages - 1}
              className="w-10 h-10 flex items-center justify-center rounded-full bg-yellow-400 text-black hover:bg-yellow-500 disabled:opacity-30 transition"
            >
              <FaArrowRight />
